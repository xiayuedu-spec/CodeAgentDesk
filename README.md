...
### 会话管理
- 多会话标签页并行运行（node-pty 承载 `claude` 进程），拖拽排序、`Ctrl+1..9` 切换、活动提醒、**标签页显示 agent 状态**
- 分组管理：分组作为会话容器（运行中 + 历史都在组内，组区块默认在上方）、8 色 + 自定义颜色、折叠、会话拖拽归属、**分组标题右键「全部归档」归档该组历史会话**
- 会话置顶/收藏（组内置顶排序 + 图钉标记）、归档/恢复、多选批量删除（应用内确认框）
- **历史会话区块标题右键「全部归档」**（只归档未分组历史会话；已分组的归各组菜单管理）
- **归档会话右键「恢复（放回历史会话）」**：只放回历史列表、不打开终端（点击归档行才是借出运行）
- 全文搜索（命中带上下文预览，点击跳转详情定位）、会话详情、导出 Markdown、打开工作目录
- **计划与改动**（会话洞察）：从会话记录里提取 TodoWrite 末次快照（进度条 + 完成率 + 快照次数）与 Edit/Write 改动清单（文件级 +N/-M、逐处片段对照），可一键复制 Markdown 摘要 —— 入口在信息面板按钮 / 详情页工具栏 / 会话右键菜单
- **终端选中文本自动复制**（带「已复制」提示；`Ctrl+C`/`Ctrl+V` 与右键菜单保留）

### 维护与治理
- **自动更新**：electron-updater + GitHub Releases，启动静默检查、后台下载、☰ 更多一键重启安装（开发模式安全降级）
- **备份 / 迁移**：导出/导入应用数据 JSON（导入前自动快照），换机器一条链路
- **功能使用统计**：本地计数（命令面板/搜索/总结/知识库/详情/导出/番茄钟…），指导功能删减与团队培训
- 长线维护路线图见 [docs/BACKLOG.md](docs/BACKLOG.md)
...
- Agent 状态标记（🧠 思考中 / ✨ 回合完成 / 😴 空闲 / 🚨 待审批）：状态栏 + 会话行 + 标签页（可切换为**单色图标**或颜色圆点）
...
- **归档/整理**：会话右键 → 归档；分组右键 → 改色/重命名/删除 + 全部归档；置顶保持高频项目在前
...
- **维护**：☰ 更多 → 检查更新 / 重启并安装更新 / 备份迁移 / 功能使用统计
...
│   ├── updater.ts         # 自动更新（electron-updater，打包环境生效）
│   ├── backup.ts          # 备份/迁移应用数据
│   ├── usage-store.ts     # 功能使用统计（本地计数）
