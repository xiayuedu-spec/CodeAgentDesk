# CodeAgentDesk 开发文档（历史设计稿）

> ⚠️ **归档说明**：本文为早期产品设计稿（2026-08-05），其中部分技术选型（better-sqlite3、@vscode/ripgrep）**未采用**（实际为 JSON 文件存储 + 内置扫描）。当前架构以 `docs/ARCHITECTURE.md` 为准，功能总览见 `README.md`，开发交接见 `agent.md`。本文保留作为产品背景与决策记录参考。
>
> Claude Code 统一窗口管理器 —— 桌面应用
>
> 日期：2026-08-05 · 状态：设计稿（已归档）
...
- 顶部全局搜索框（切到"搜索"视图）。
- 底部模式切换：[会话] / [搜索]。
- **会话列表**：当前/历史两组，组内按 `last_activity_at` 倒序。每行显示：标题（自定义名 → 目录名）、工作目录、相对时间、状态圆点（● 运行中 / ○ 已停止），hover 显示完整路径。右键菜单：重命名 / 归档 / 恢复 / 查看详情 / 复制内容 / 复制路径。
- 侧边栏可拖拽调宽（180–480px）；会话列表支持 ↑/↓ + Enter 键盘导航（未按键不高亮）。
- 会话列表实时刷新（watcher 新增/删除 → `sessionsChanged`）；新建按钮弹最近目录快速开会话；未激活标签有新输出时 dot 高亮（`has-activity`）。
- （路线图）按项目分组 + 会话数角标尚未实现。
- **新建会话**按钮：目录选择。

### 主内容区
- **终端面板**：xterm.js，每个标签一个实例；pty 数据经 IPC 流式转发。
- **欢迎页/空状态**：无激活会话时显示品牌引导卡（新建会话 / 打开历史 / 快捷键提示）。
- **会话详情视图**：虚拟化列表渲染 JSONL 为聊天流——用户消息、assistant 消息（Markdown + 代码高亮）、工具调用折叠卡片（工具名 / 输入 / 输出）。按钮：复制消息、导出当前会话为 Markdown。
- **搜索视图**：ripgrep 命中按会话分组，显示命中片段（关键词 `<mark>` 高亮、搜索框可一键清空），点击跳转详情或"恢复会话"。
- **命令面板（Ctrl+P）**：新建/最近目录/恢复历史/归档/全文搜索/导出/切主题/设置，输入过滤 + ↑↓/Enter。
- **AI 摘要**：会话结束时自动生成摘要+标签（列表行显示标签/摘要），详情视图 ✨ 可手动；**总结体系**：今日/月度总结（聚合当日/当月会话出 Markdown）生成即归档到 `summaries.json`，模态三标签（今日/月度/历史）可查看历史全文。均走 `claude -p` 无头；会话摘要存 `session-meta.json`。
- **拖放目录开会话**：拖文件夹到窗口出现虚线 overlay，释放即在目录开会话。

### 视觉基调（方向 A）
深色青绿主题为主，共 6 套可切换主题：深色默认 / Mac 浅色 / 护眼豆沙绿 / 暖纸米黄 / 琥珀夜间 / 柔雾深青（终端配色随主题联动、窗口底色同步）。精修点（已落地）：面板内顶高光 + 柔和阴影的层次感、hover/焦点环；主题强调色全部 var() 化（glow/soft/selection/CTA 渐变），新增主题只加变量块。微交互（hover 上浮/按压反馈/标签切换淡入）、会话列表骨架屏、标签拖拽排序已落地。会话标题用 ai-title、标签栏"贴顶无边框"两版方案均试做后回退，维持原样。
