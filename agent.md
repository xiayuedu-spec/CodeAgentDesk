...
- **Token 限额预警**：主进程每 5 分钟检查**当前自然小时**（整点起，额度整点刷新）消耗（`getCurrentHourUsage`，会话按 updatedAt 归入所在小时，复用增量缓存），达限额 80%/100% 时系统通知（每档每小时一次，跨小时自动重置）；默认限额 1000 万/小时，可用 `config.json#tokenLimitPerHour` 覆盖；Dashboard/欢迎页显示本小时消耗进度条（80% 黄 / 100% 红）
